...
void ft_builtins(t_core *core)
{
	...
	{
		if (!ft_strncmp(core->lexer->content, "echo", 4) && ft_strlen(core->lexer->content) == 4)
			ft_echo_management(core);
		else if (!ft_strncmp(core->lexer->content, "unset", 5) && ft_strlen(core->lexer->content) == 5)
			ft_unset_management(core);
		else if (!ft_strncmp(core->lexer->content, "exit", 4) && ft_strlen(core->lexer->content) == 4)
			ft_exit_management(core);
		else if (!ft_strncmp(core->lexer->content, "pwd", 3) && ft_strlen(core->lexer->content) == 3)
			ft_pwd_management(core);
		else if (!ft_strncmp(core->lexer->content, "env", 3) && ft_strlen(core->lexer->content) == 3)
			ft_env_management(core);
		else if (!ft_strncmp(core->lexer->content, "cd", 2) && ft_strlen(core->lexer->content) == 2)
			ft_chdir(core);
